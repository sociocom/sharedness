...
df = df.dropna(subset=["individual well-being", "TSI"])

# === Overall correlation ===
r_all, p_all = pearsonr(df["individual well-being"], df["TSI"])
print("Overall correlation (individual well-being vs. TSI):")
...
for team, group in df.groupby("team"):
    if group.shape[0] > 1:  # Need at least 2 data points to compute correlation
        r, p = pearsonr(group["individual well-being"], group["TSI"])
